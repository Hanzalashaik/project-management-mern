...
//POST admin
router.post("/register",userRegisterValidations(),
...
router.post("/login", async (req, res) => {
  try {
    ...
    const isPasswordValid = await adminModel.findOne({ password });

    if (!isPasswordValid) {
      return res.status(400).json({ msg: "Incorrect Password" });
    }

    res.status(201).json({ msg: "Logged In Successfully" ,admin});
  } catch (error) {
    console.log(error);
